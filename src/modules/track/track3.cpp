...
AddTrackSurface(void *TrackHandle, tTrack *theTrack, const char *material)
{
	...
	curSurf = trackFindSurface(theTrack, material);
	if (curSurf != nullptr) {
		return curSurf;
	}
	
	/* Create a new surface */
	curSurf = trackAppendSurface(theTrack, material, "AddTrackSurface");
	snprintf(path, BUFSIZE, "%s/%s/%s", TRK_SECT_SURFACES, TRK_LST_SURF, material);
	trackLoadSurfaceProperties(curSurf, TrackHandle, path, 1.0f);
...
InitSides(void *TrackHandle, tTrack *theTrack)
{
	...
	for (side = 0; side < 2; side++) {
		...
		sideEndWidth[side] = GfParmGetNum(TrackHandle, TRK_SECT_MAIN, KeySideWidth[side], nullptr, 0.0);

		/* banking of borders */
		sideBankType[side] = trackParseBankingType(
			GfParmGetStr(TrackHandle, TRK_SECT_MAIN, KeySideBankType[side], TRK_VAL_LEVEL));
		...
		borderWidth[side] = GfParmGetNum(TrackHandle, TRK_SECT_MAIN, KeyBorderWidth[side], nullptr, 0.0);
		borderHeight[side] = GfParmGetNum(TrackHandle, TRK_SECT_MAIN, KeyBorderHeight[side], nullptr, 0.0);
		const char* style = GfParmGetStr(TrackHandle, TRK_SECT_MAIN, KeyBorderStyle[side], TRK_VAL_PLAN);
		borderStyle[side] = trackParseBorderStyle(style);
		...
		barrierHeight[side] = GfParmGetNum(TrackHandle, TRK_SECT_MAIN, KeyBarrierHeight[side], nullptr, 1.0);
		style = GfParmGetStr(TrackHandle, TRK_SECT_MAIN, KeyBarrierStyle[side], TRK_VAL_FENCE);
		barrierStyle[side] = trackParseBarrierStyle(style);
		if (barrierStyle[side] == TR_FENCE) {
			barrierWidth[side] = 0;
		} else {
			barrierWidth[side] = GfParmGetNum(TrackHandle, TRK_SECT_MAIN, KeyBarrierWidth[side], nullptr, 0.5);
		}
	}
}
...
AddSides(tTrackSeg *curSeg, void *TrackHandle, tTrack *theTrack, int curStep, int steps)
{
	...
	if (curStep == 0) {
	    /* Side parameters */
	    sw = GfParmGetCurNum(TrackHandle, path, KeySideStartWidth[side], nullptr, sideEndWidth[side]);
	    ew = GfParmGetCurNum(TrackHandle, path, KeySideEndWidth[side], nullptr, sw);
	    ...
	    /* Border parameters */
	    bw = GfParmGetCurNum(TrackHandle, path, KeyBorderWidth[side], nullptr, borderWidth[side]);
	    borderWidth[side] = bw;
	    borderHeight[side] = GfParmGetCurNum(TrackHandle, path, KeyBorderHeight[side], nullptr, 0.0);
	    borderMaterial[side] = GfParmGetCurStr(TrackHandle, path, KeyBorderSurface[side], borderMaterial[side]);
	    borderSurface[side] = AddTrackSurface(TrackHandle, theTrack, borderMaterial[side]);
	    const char* style = GfParmGetCurStr(TrackHandle, path, KeyBorderStyle[side],
						trackBorderStyleValue(borderStyle[side]));
	    borderStyle[side] = trackParseBorderStyle(style);

	    /* Barrier parameters */
	    barrierMaterial[side] = GfParmGetStr(TrackHandle, TRK_SECT_MAIN, KeyBarrierSurface[side], barrierMaterial[side]);
	    barrierSurface[side] = AddTrackSurface(TrackHandle, theTrack, barrierMaterial[side]);
	    barrierHeight[side] = GfParmGetNum(TrackHandle, TRK_SECT_MAIN, KeyBarrierHeight[side], nullptr, barrierHeight[side]);
	    style = GfParmGetStr(TrackHandle, TRK_SECT_MAIN, KeyBarrierStyle[side],
				 trackBarrierStyleValue(barrierStyle[side]));
	    barrierStyle[side] = trackParseBarrierStyle(style);
	    if (barrierStyle[side] == TR_FENCE) {
		barrierWidth[side] = 0;
	    } else {
		barrierWidth[side] = GfParmGetNum(TrackHandle, TRK_SECT_MAIN, KeyBarrierWidth[side], nullptr, barrierWidth[side]);
	    }
	} else {
	...
	/* Borders */
	if (bw != 0.0) {
	    curBorder = trackCalloc<tTrackSeg>("AddSides border");
	    if (side == 1) {
	    ...
	    curBorder->angle[TR_XS] = curSeg->angle[TR_XS] * static_cast<tdble>(type);
	    curBorder->angle[TR_XE] = curSeg->angle[TR_XE] * static_cast<tdble>(type);
		...
		switch(side) {
		case 1:
		    curBorder->radius = static_cast<tdble>(curSeg->radiusl - bw / 2.0);
		    ...
		    al = static_cast<tdble>((curBorder->angle[TR_ZE] - curBorder->angle[TR_ZS]) / 36.0);
		...
		case 0:
		    curBorder->radius = static_cast<tdble>(curSeg->radiusr + bw / 2.0);
	...
	/* Sides */
	if ((sw != 0.0) || (ew != 0)) {
	    curSide = trackCalloc<tTrackSeg>("AddSides side");
	    if (side == 1) {
		...
		switch(side) {
		case 1:
		    curSide->radius = static_cast<tdble>(curSeg->radiusl - sw / 2.0);
		    ...
		    /* to find the boundary */
		    al = static_cast<tdble>((curSide->angle[TR_ZE] - curSide->angle[TR_ZS]) / 36.0);
		...
		case 0:
		    curSide->radius = static_cast<tdble>(curSeg->radiusr + sw / 2.0);
...
#define MAX_TMP_INTS	256
    ...
    do {
		...
		if (ext) {
			...
			ind = trackParseMarkIds(marks, mi, MAX_TMP_INTS);
		}
		...
		if (strcmp(segtype, TRK_VAL_STR) == 0) {
			/* straight */
			length = GfParmGetCurNum(TrackHandle, path, TRK_ATT_LG, nullptr, 0);
			type = TR_STR;
			radius = radiusend = 0;
		} else if (strcmp(segtype, TRK_VAL_LFT) == 0) {
			/* left curve */
		...
		zsl = GfParmGetCurNum(TrackHandle, path, TRK_ATT_ZSL, nullptr, zsl);
		zsr = GfParmGetCurNum(TrackHandle, path, TRK_ATT_ZSR, nullptr, zsr);
		zel = GfParmGetCurNum(TrackHandle, path, TRK_ATT_ZEL, nullptr, zel);
		zer = GfParmGetCurNum(TrackHandle, path, TRK_ATT_ZER, nullptr, zer);
		ze = zs = -100000.0;
		ze = GfParmGetCurNum(TrackHandle, path, TRK_ATT_ZE, nullptr, ze);
		zs = GfParmGetCurNum(TrackHandle, path, TRK_ATT_ZS, nullptr, zs);
		grade = GfParmGetCurNum(TrackHandle, path, TRK_ATT_GRADE, nullptr, grade);
		if (zs != -100000.0) {
			zsr = zsl = zs;
		} else {
		...
		if (strcmp(profil, TRK_VAL_SPLINE) == 0) {
			...
			stgtl = GfParmGetCurNum(TrackHandle, path, TRK_ATT_PROFTGTSL, nullptr, stgtl);
			etgtl = GfParmGetCurNum(TrackHandle, path, TRK_ATT_PROFTGTEL, nullptr, etgtl);
			stgtr = GfParmGetCurNum(TrackHandle, path, TRK_ATT_PROFTGTSR, nullptr, stgtr);
			etgtr = GfParmGetCurNum(TrackHandle, path, TRK_ATT_PROFTGTER, nullptr, etgtr);
			stgt = etgt = -100000.0;
			stgt = GfParmGetCurNum(TrackHandle, path, TRK_ATT_PROFTGTS, nullptr, stgt);
			etgt = GfParmGetCurNum(TrackHandle, path, TRK_ATT_PROFTGTE, nullptr, etgt);
			if (stgt != -100000.0) {
				stgtl = stgtr = stgt;
			}
			if (etgt != -100000.0) {
		...
		while (curStep < steps) {
			...
			curSeg = trackCalloc<tTrackSeg>("CreateSegRing3 segment");
			if (root == nullptr) {
				root = curSeg;
				curSeg->next = curSeg;
				curSeg->prev = curSeg;
			...
			if (ext && ind) {
				curSeg->ext = trackCreateMarkExtension(mi, ind, "CreateSegRing3 extension");
				ind = 0;
			}
			...
			/* to find the boundary */
			al = static_cast<tdble>((curSeg->angle[TR_ZE] - curSeg->angle[TR_ZS]) / 36.0);
...
ReadTrack3(tTrack *theTrack, void *TrackHandle, tRoadCam **camList, int ext)
{
    ...
    GlobalStepLen = GfParmGetNum(TrackHandle, TRK_SECT_MAIN, TRK_ATT_PROFSTEPSLEN, nullptr, 0);

    CreateSegRing3(TrackHandle, theTrack, nullptr, nullptr, ext);
	...
	pits->speedLimit = GfParmGetNum(TrackHandle, TRK_SECT_MAIN, TRK_ATT_SPD_LIM, nullptr, 25.0);
	if ((pitEntrySeg != nullptr) && (pitExitSeg != nullptr) && (pitStart != nullptr) && (pitEnd != nullptr)) {
	    ...
	    pits->len   = GfParmGetNum(TrackHandle, TRK_SECT_MAIN, TRK_ATT_PIT_LEN, nullptr, 15.0);
	    pits->width = GfParmGetNum(TrackHandle, TRK_SECT_MAIN, TRK_ATT_PIT_WIDTH, nullptr, 5.0);
	    found = true;
	} else {
	    found = false;
	}
	...
	if (pitStart->lgfromstart > pitEnd->lgfromstart) {
	    ...
	    pits->nMaxPits = static_cast<int>((- pitStart->lgfromstart + pitEnd->lgfromstart +
					     pitEnd->length + pits->len / 2.0) / pits->len);
	}
	pits->driversPits = trackCallocArray<tTrackOwnPit>(pits->nMaxPits, "ReadTrack3 pits");
	...
	do {
	    curCam = trackAppendCamera(camList, "ReadTrack3 camera");
	    ...
	    trkPos.seg = curSeg;
	    trkPos.toRight = GfParmGetCurNum(TrackHandle, path, TRK_ATT_TORIGHT, nullptr, 0);
	    trkPos.toStart = GfParmGetCurNum(TrackHandle, path, TRK_ATT_TOSTART, nullptr, 0);
	    TrackLocal2Global(&trkPos, &(curCam->pos.x), &(curCam->pos.y));
	    curCam->pos.z = TrackHeightL(&trkPos) + GfParmGetCurNum(TrackHandle, path, TRK_ATT_HEIGHT, nullptr, 0);
