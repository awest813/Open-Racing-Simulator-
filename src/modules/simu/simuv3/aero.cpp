...
SimAeroConfig(tCar *car)
{
    ...
    Cx       = GfParmGetNum(hdle, SECT_AERODYNAMICS, PRM_CX, nullptr, 0.4);
    FrntArea = GfParmGetNum(hdle, SECT_AERODYNAMICS, PRM_FRNTAREA, nullptr, 2.5);
    car->aero.Clift[0] = GfParmGetNum(hdle, SECT_AERODYNAMICS, PRM_FCL, nullptr, 0.0);
    car->aero.Clift[1] = GfParmGetNum(hdle, SECT_AERODYNAMICS, PRM_RCL, nullptr, 0.0);
...
void SimAeroDamage(tCar *car, sgVec3 poc, tdble F)
{
    ...
    if (sgScalarProductVec3(car->aero.rot_front, car->aero.rot_front) > 1.0) {
    ...
    if (sgScalarProductVec3(car->aero.rot_lateral, car->aero.rot_lateral) > 1.0) {
    ...
    if (sgScalarProductVec3(car->aero.rot_vertical, car->aero.rot_vertical) > 1.0) {
