...
## 2024-05-24 - Accessibility in Admin Event Creation
**Learning:** Legacy template forms, like the one in `admin_event_create.ihtml`, lack structured `<label>` to form field associations, which creates navigation difficulties for screen reader users.
**Action:** Adding explicit `<label for="...">` alongside `id="..."` attributes on the input and select tags corrects this structural oversight, improving semantic relationships and accessibility without requiring CSS adjustments.
