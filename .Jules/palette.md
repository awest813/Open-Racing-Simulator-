...
## 2024-07-02 - Registration Form Autocomplete Attributes
**Learning:** Legacy `.ihtml` registration forms often miss `autocomplete` attributes on standard profile fields (like first name, last name, and new passwords), forcing users to manually type values that password managers or browser autofill could supply.
**Action:** When updating registration forms, apply semantic HTML5 `autocomplete` attributes (e.g., `given-name`, `family-name`, `new-password`, `email`) along with `required`. This unlocks seamless browser autofill support and improves account creation efficiency without needing new styles or scripts.
