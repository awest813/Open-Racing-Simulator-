...
## 2024-12-04 - Adding Labels Across Table Cells
**Learning:** In legacy tabular layouts (like `torcs_racing_board`), form inputs and their descriptive text are often separated into adjacent `<td>` cells. Wrapping the descriptive text in a `<label for="id">` crosses the cell boundary, effectively turning the entire text block into a large clickable target, drastically improving usability, especially for tiny inputs like checkboxes.
**Action:** When improving forms with tabular layouts, ensure `id`s are added to inputs and `<label>` tags are wrapped around the text in adjacent cells.
