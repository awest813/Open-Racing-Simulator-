...
## 2024-05-24 - Enhance Form Autocomplete Attributes
**Learning:** Missing autocomplete attributes on registration and account management forms prevent browser password managers and autofill features from working optimally, introducing unnecessary friction for users filling out their details.
**Action:** Add appropriate semantic autocomplete attributes (`username`, `given-name`, `family-name`, `new-password`, `email`) to all standard user detail forms to support modern browser autofill and password manager integrations.
