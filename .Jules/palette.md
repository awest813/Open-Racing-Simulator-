...
## 2024-06-10 - Expanded clickable area for quiz answers
**Learning:** In legacy tabular layouts, separating radio buttons and their labels into different table cells (`<td>`) reduces the clickable target area and makes selection harder, especially on touch devices or for users with motor impairments.
**Action:** Cross `<td>` boundaries by consolidating the input and text into a single cell and wrapping both inside the `<label for="...">` tag to maximize the hit area without needing custom CSS.
