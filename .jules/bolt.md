...
## 2024-05-24 - Defer expensive atan2 evaluation
**Learning:** In C++ aerodynamics simulations containing `atan2` functions before high-probability early-exit conditions (such as velocity or bounds checks), execution paths can be bottlenecked by computing expensive math operations unnecessarily.
**Action:** Defer the expensive math operations (like `atan2`) inside the conditional blocks where their results are explicitly needed, instead of calculating unconditionally.
