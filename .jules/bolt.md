...
## 2024-05-18 - Deferred Trigonometry Optimization in Aero Hot Paths
**Learning:** Unconditionally calculating expensive trigonometric functions like `atan2()` before evaluating fast early-exit bounds checks (like distance thresholds) causes measurable performance bottlenecks in C++ hot loops (e.g., iterating over opponents in aero calculations).
**Action:** Always defer expensive mathematical operations by computing squared distance thresholds first, and only calculating functions like `atan2()` strictly inside the conditional blocks where their results are actually needed and drafting effects matter.
