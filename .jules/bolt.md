...
## 2024-05-18 - Optimize Vector Length Comparisons
**Learning:** In TORCS physics code, `sgLengthVec3` computes expensive square roots. When comparing magnitudes or thresholds (like `1.0`), this is redundant.
**Action:** Replace `sgLengthVec3()` with `sgLengthSquaredVec3()` and square the threshold (e.g. `threshold * threshold`) to bypass square root calculations in hot loops.
