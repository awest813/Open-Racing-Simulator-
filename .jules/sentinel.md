...
## 2024-05-27 - Reflected XSS in Event Registration and Account Management
**Vulnerability:** Reflected XSS via unescaped `$_SERVER['PHP_SELF']` assignments to template variables in `event_register_team.php` and `lostpassword.php`.
**Learning:** Raw assignments of `$_SERVER['PHP_SELF']` directly to `.ihtml` template engines are prone to Reflected XSS because the value contains unescaped user-controlled URL components.
**Prevention:** Wrap `$_SERVER['PHP_SELF']` with `htmlspecialchars($var, ENT_QUOTES, 'UTF-8')` before assignment to template contexts.
