...
## 2024-05-18 - Prevent Reflected XSS in Template Variables
**Vulnerability:** The legacy `torcs_racing_board` PHP application widely passes `$_SERVER['PHP_SELF']` directly to `.ihtml` template variables (e.g., `PS_LOGINPAGE`, `PC_ACCOUNTPAGE`, `PC_USER_PAGE`) without any sanitization. This allows an attacker to inject arbitrary HTML or JavaScript via the URL path (Reflected XSS), which gets reflected in `<form action="...">` or `<a href="...">` tags.
**Learning:** `$_SERVER['PHP_SELF']` is generally unsafe to echo or assign to templates directly because it contains user-controlled input (the path info). Using it without escaping creates a high-severity XSS vulnerability, especially in older PHP applications that lack modern routing mechanisms.
**Prevention:** Always wrap `$_SERVER['PHP_SELF']` (and other dynamic URL components like `$_SERVER['QUERY_STRING']`) in `htmlspecialchars($var, ENT_QUOTES, 'UTF-8')` before assignment or output to ensure special characters like quotes and angle brackets are correctly encoded.
