...
## 2024-04-15 - Reflected XSS in legacy PHP templates via `$_SERVER['PHP_SELF']`
**Vulnerability:** Reflected Cross-Site Scripting (XSS) due to unescaped usage of `$_SERVER['PHP_SELF']` directly interpolated into HTML attributes via template variables in `.ihtml` template engines.
**Learning:** `$_SERVER['PHP_SELF']` contains user-controlled input (like URL paths) and is inherently unsafe. Older codebases (e.g. PHP 5.3) frequently pass these unsanitized directly into `.ihtml` views, causing them to execute inside `href` or `action` attributes on legacy template parses, bypassing modern output escaping.
**Prevention:** Always wrap dynamically injected server variables (like `$_SERVER['PHP_SELF']` or `QUERY_STRING`) with `htmlspecialchars($var, ENT_QUOTES, 'UTF-8')` immediately during template variable assignment.
