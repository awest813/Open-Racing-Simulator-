...
## 2024-05-24 - Reflected XSS via Unescaped PHP_SELF in .ihtml Templates
**Vulnerability:** Widespread usage of `$_SERVER['PHP_SELF']` without any escaping when assigning template variables like `PS_LOGINPAGE`.
**Learning:** `$_SERVER['PHP_SELF']` is often placed in `action="..."` or `href="..."` attributes and can be manipulated by appending path info (`/foo.php/"><script>alert(1)</script>`), enabling XSS.
**Prevention:** Always wrap `$_SERVER['PHP_SELF']` and similar server variables in `htmlspecialchars($var, ENT_QUOTES, 'UTF-8')` before using them.
