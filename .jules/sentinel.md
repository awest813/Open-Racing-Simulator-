...
## 2024-05-24 - Sanitize $_SERVER['PHP_SELF'] to Prevent Reflected XSS
**Vulnerability:** Assigning `$_SERVER['PHP_SELF']` to `.ihtml` template variables (like `PC_CREATETEAMPAGE` and `PS_LOGINPAGE`) without sanitization exposes the application to Reflected Cross-Site Scripting (XSS).
**Learning:** `$_SERVER['PHP_SELF']` contains user-controlled input (the requested URI) and can be manipulated by attackers to inject malicious scripts into the rendered template.
**Prevention:** Always sanitize `$_SERVER['PHP_SELF']` using `htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8')` before using it in HTML output or template assignments, ensuring that quotes are escaped and preventing XSS payloads.
