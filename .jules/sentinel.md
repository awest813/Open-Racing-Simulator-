...
## 2026-04-12 - SQL Injection via unquoted table and column names
**Vulnerability:** SQL injection vulnerabilities existed in legacy database functions (`existsTable`, `existsEntry`, `countHit`, `countSession`) where table and column names were directly concatenated into queries without proper quoting.
**Learning:** Functions meant for general database access that dynamically construct queries using user-provided or variable table/column names are highly susceptible to SQL injection. In environments where parameterization for identifiers isn't available, specific helper functions like `quoteIdentifier()` must be used to sanitize them.
**Prevention:** Always use `quoteIdentifier()` (or equivalent functionality to wrap names in backticks and escape/strip slashes) when substituting table or column names dynamically into SQL queries.
