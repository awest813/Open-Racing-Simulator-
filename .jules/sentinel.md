...
## 2024-05-27 - Reflected XSS in PHP_SELF Variable Assignment
**Vulnerability:** Found unescaped output of `$_SERVER['PHP_SELF']` to the frontend using `.ihtml` template assignments in `torcs_racing_board/teams/team_create.php`. An attacker could exploit this by crafting a malicious URL containing JavaScript payloads in the path, leading to Reflected XSS when the path is echoed back in HTML attributes like `action`.
**Learning:** Legacy PHP code uses `.ihtml` templates and `set_var` assignments but relies on raw `$_SERVER['PHP_SELF']` without sanitization. Single quotes in the URL could break out of HTML attributes if not properly escaped.
**Prevention:** Always wrap `$_SERVER['PHP_SELF']` (and other URL-derived values like `QUERY_STRING`) with `htmlspecialchars($var, ENT_QUOTES, 'UTF-8')` before setting it in a frontend variable or template in older PHP stacks.
