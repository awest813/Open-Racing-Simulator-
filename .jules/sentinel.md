...
**Vulnerability:** Unescaped output of $_SERVER['PHP_SELF'] in lostpassword.php.
**Learning:** Template assignments like PC_LOSTPASSWORDPAGE using $_SERVER['PHP_SELF'] are vulnerable to XSS if not properly escaped.
**Prevention:** Wrap $_SERVER['PHP_SELF'] with htmlspecialchars($var, ENT_QUOTES, 'UTF-8').
