...
## 2024-05-18 - Reflected XSS via `$_SERVER['PHP_SELF']`
**Vulnerability:** Widespread Reflected XSS vulnerabilities exist due to the unescaped use of `$_SERVER['PHP_SELF']` when constructing dynamic URLs that are rendered in `.ihtml` templates (e.g. `torcs_racing_board/teams/teamlist.php`).
**Learning:** Default PHP installations may not sanitize `PHP_SELF` automatically. When directly assigning it to template variables, attackers can inject malicious scripts by appending payloads to the URL (e.g. `teamlist.php/"><script>alert(1)</script>`), which gets reflected in the `href` or `action` attributes of links and forms.
**Prevention:** Always escape dynamic URL components like `$_SERVER['PHP_SELF']` using `htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8')` before using them in templates or output.
