...
## 2024-05-24 - Unescaped PHP_SELF Reflected XSS
**Vulnerability:** Unescaped usage of `$_SERVER['PHP_SELF']` when generating HTML output in multiple files leads to Reflected Cross-Site Scripting (XSS) vulnerabilities.
**Learning:** Legacy PHP code often assigns raw `$_SERVER['PHP_SELF']` (or `$_SERVER['QUERY_STRING']`) into `.ihtml` template variables, which then gets injected directly into HTML attributes (like `<form action="...">` or `<a href="...">`). An attacker can exploit this by appending a malicious payload directly to the URL.
**Prevention:** Always wrap variables generated from the URL (like `$_SERVER['PHP_SELF']` and `$_SERVER['QUERY_STRING']`) using `htmlspecialchars($var, ENT_QUOTES, 'UTF-8')` before using them in an HTML context, ensuring single quotes are also safely escaped.
