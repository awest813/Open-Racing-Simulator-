...
## 2024-05-24 - SQL Injection in existsEntry
**Vulnerability:** SQL Injection in `existsEntry` utility function.
**Learning:** `existsEntry` was constructing SQL queries by directly concatenating `$identifier` and `$value` parameters without escaping them. This function is used widely across the application (e.g. validating admin operations via `$_GET`/`$_POST`), presenting a critical attack surface. Even if some inputs are cast to integers, the function fundamentally lacked defense-in-depth, relying on the caller for sanitization.
**Prevention:** In legacy PHP codebases where PDO/parameterized queries are unavailable, always use existing internal helper functions like `quoteIdentifier()` and `quoteString()` to sanitize table/column names and values inside query-building utilities to prevent SQL injection vulnerabilities globally.
