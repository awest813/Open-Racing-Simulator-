...
*/
	...
	require_once($path_to_root . 'lib/functions_lostpassword.php');
	...
	cleanupPendingPasswordRequests($lostpwd_tablename);

	if ($_SESSION['logged'] == TRUE) {
		// Login template for statusbar.
		$page_statusbar = 'page_statusbar_logged_in.ihtml';
		$page_content = 'lostpassword_logged_in.ihtml';
	} else {
		// Status view template for statusbar.
		$page_statusbar = 'page_statusbar.ihtml';
		if (requestNewPassword($user_tablename, $lostpwd_tablename, $path_to_root)) {
			$page_content = 'lostpassword_request_sent.ihtml';
		} elseif (sendNewPassword($user_tablename, $lostpwd_tablename, $path_to_root)) {
			$page_content = 'lostpassword_sent.ihtml';
		} else {
			$page_content = 'lostpassword_request.ihtml';
		}
	}
