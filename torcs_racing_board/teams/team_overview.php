...
*/
	...
	// Set up page header.
	$page->set_var(array(
		'PB_PAGETITLE'		=> 'Open Racing Simulator — Racing Board Teams',
		'PB_DESCRIPTION'	=> 'Teams on the Open Racing Simulator racing board',
	...
	$page->set_var(array(
		'PC_LIST_NEXT_LINK'		=> ($more_rows === TRUE) ? htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?listfrom=' . $lastimagerow : '',
		'PC_LIST_NEXT'			=> ($more_rows === TRUE) ? 'Next' : '',
		//'PC_LIST_PREV_LINK'		=> ($listfrom > 0) ? htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?listback=' . max($listfrom-IMAGELISTENTRIES, 0) : '',
