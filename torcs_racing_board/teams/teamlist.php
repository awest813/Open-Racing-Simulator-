...
*/
	...
	// Set up page header.
	$page->set_var(array(
		'PB_PAGETITLE'		=> 'Open Racing Simulator — Racing Board Team List',
		'PB_DESCRIPTION'	=> 'Team list of the Open Racing Simulator racing board',
		'PB_AUTHOR'			=> 'Bernhard Wymann',
		'PB_KEYWORDS'		=> 'Open Racing Simulator, racing, berniw, Bernhard, Wymann, Championship, World, Board, Team, Teams, List',
	...
	while (($myrow = mysql_fetch_array($result)) && $results < LISTENTRIES) {
		$page->set_var(array(
			...
			'PC_TEAM_DELETE_HREF'	=> (($_SESSION['usergroup'] == 'admin')) ? htmlspecialchars($_SERVER['PHP_SELF'] . '?deleteteamid=' . $myrow['teamid'], ENT_QUOTES, 'UTF-8'): ''
		));
