...
*/
	...
	deleteSShot($sshot_tablename, $path_to_root);

	$page_content = 'gallery.ihtml';
	...
	// Set up page header.
	$page->set_var(array(
		'PB_PAGETITLE'		=> 'Open Racing Simulator Screenshots',
		'PB_DESCRIPTION'	=> 'Open Racing Simulator screenshots',
		'PB_AUTHOR'			=> 'Bernhard Wymann',
		'PB_KEYWORDS'		=> 'Open Racing Simulator, racing, berniw, Bernhard, Wymann, Championship, World, Board, screenshots',
	...
	$page->set_block('col', 'sshotadmin', 'sshotadminvar');
	...
	$page->set_var(array(
		'PC_LIST_NEXT_LINK'		=> ($more_rows === TRUE) ? htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?listfrom=' . ($listfrom+$LST) : '',
		'PC_LIST_NEXT'			=> ($more_rows === TRUE) ? 'Next' : '',
		'PC_LIST_PREV_LINK'		=> ($listfrom > 0) ? htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?listfrom=' . max($listfrom-$LST, 0) : '',
	...
	while (($myrow = mysql_fetch_array($result)) && ($count < $LST)) {
		$page->set_var(array(
			...
			'PC_SSHOT_IMG_SRC'		=> $path_to_root . 'images/screenshots/' . $myrow['id'] . '_tn.jpg'
		));
		if ($_SESSION['usergroup'] == 'admin') {
			$page->parse('sshotadminvar', 'sshotadmin');
		} else {
			$page->set_var('sshotadminvar', '');
		}
