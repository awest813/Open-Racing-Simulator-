...
*/
	...
	// Set up page header.
	$page->set_var(array(
		'PB_PAGETITLE'		=> 'Open Racing Simulator — Racing Board Screenshot View',
		'PB_DESCRIPTION'	=> 'View a screenshot on the Open Racing Simulator racing board',
		'PB_AUTHOR'			=> 'Bernhard Wymann',
		'PB_KEYWORDS'		=> 'Open Racing Simulator, racing, berniw, Bernhard, Wymann, Championship, World, Board, View, Show, Screenshot',
	...
	if (/*isset($_GET['viewshotid']) &&*/ isset($_GET['shotrowid'])) {
		...
		$page->set_var(array(
			'PC_LIST_NEXT_LINK'		=> ($nextimg) ? htmlspecialchars($_SERVER['PHP_SELF'] . '?shotrowid=' . ($rowid+1), ENT_QUOTES, 'UTF-8'): '',
			'PC_LIST_NEXT'			=> ($nextimg) ? 'Next' : '',
			'PC_LIST_PREV_LINK'		=> ($previmg) ? htmlspecialchars($_SERVER['PHP_SELF'] . '?shotrowid=' . ($rowid-1), ENT_QUOTES, 'UTF-8'): '',
