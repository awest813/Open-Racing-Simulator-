...
function countHit($page, $table)
{
	...
	$table_quoted = quoteIdentifier($table);
	// If it exists update.
	$sql = "UPDATE " . $table_quoted . " SET hits=hits+1 WHERE page=" . $page_for_db;
	mysql_query($sql);
	if (mysql_affected_rows() == 0) {
		// Create.
		$sql = "INSERT INTO " . $table_quoted . " (page, hits) VALUES " .
			   "($page_for_db, '1')";
		mysql_query($sql);
	}
}
...
{
	$table_quoted = quoteIdentifier($table);
	$stats_table_quoted = quoteIdentifier($stats_table);

	// Delete outdated sessions.
	$date_expire_for_db = quoteString(date("YmdHis", time() - SESSION_STATS_EXPIRE));
	$sql = "DELETE FROM " . $table_quoted . " WHERE start < " . $date_expire_for_db;
	mysql_query($sql);

	// Check if session exists.
	$session_for_db = quoteString($session);
	$sql = "SELECT * FROM " . $table_quoted . " WHERE session=" . $session_for_db;
	$result = mysql_query($sql);
	if (mysql_num_rows($result) == 0) {
		$sql = "INSERT INTO " . $table_quoted . " (session) VALUES ($session_for_db)";
		mysql_query($sql);
		// Update visits counter.
		$sql = "UPDATE " . $stats_table_quoted . " SET value=value+1 WHERE name='visits'";
		mysql_query($sql);
	}
}
