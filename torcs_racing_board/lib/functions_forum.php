		...
		while ($myrow = mysql_fetch_array($result)) {
			...
			if ($_SESSION['usergroup'] == 'admin') {
				...
				if ($myrow['id_parent'] == 0) {
					...
					$delete_href = htmlspecialchars($_SERVER['PHP_SELF'] . '?deleteid=' . $myrow['id'] . '&topicid=' . $myrow['id_thread'], ENT_QUOTES, 'UTF-8');
				}
			} else {
				$delete = '';
				$delete_href = '';
			...
			if ($myrow['state'] == 'visible') {
				...
				if ($_SESSION['usergroup'] == 'admin') {
					$hide_href = htmlspecialchars($_SERVER['PHP_SELF'] . '?msgid=' . $myrow['id'] .
								 '&hide=true&topicid=' . $myrow['id_thread'], ENT_QUOTES, 'UTF-8');
					$hide = 'Hide';
				} else {
					$hide_href = '';
					$hide = '';
				...
				if ($_SESSION['usergroup'] == 'admin') {
					...
					$display_subject = htmlentities($myrow['subject']);
					$hide_href = htmlspecialchars($_SERVER['PHP_SELF'] . '?msgid=' . $myrow['id'] .
								 '&hide=false&topicid=' . $myrow['id_thread'], ENT_QUOTES, 'UTF-8');
					$hide = 'Show';
				} else {
					$hide = '';
					$hide_href = '';
