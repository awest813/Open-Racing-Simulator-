...
*/
	...
	$page_content = 'car_overview.ihtml';
	...
	// Set up page header.
	$page->set_var(array(
		'PB_PAGETITLE'		=> 'Open Racing Simulator — Racing Board Car Gallery',
		'PB_DESCRIPTION'	=> 'Car gallery of the Open Racing Simulator racing board',
		'PB_AUTHOR'			=> 'Bernhard Wymann',
		'PB_KEYWORDS'		=> 'Open Racing Simulator, racing, berniw, Bernhard, Wymann, Championship, World, Board, Cars, Gallery',
	...
	$sql = "SELECT * FROM $car_tablename " .
		   "ORDER BY train ASC, name ASC LIMIT $listfrom_for_db,$listentries_for_db";
	$result = mysql_query($sql);

	$more_rows = (mysql_num_rows($result) > OVRIMGENTRIES);
	$page->set_var(array(
		'PC_LIST_NEXT_LINK'		=> ($more_rows === TRUE) ? htmlspecialchars($_SERVER['PHP_SELF'] . '?listfrom=' . ($listfrom+OVRIMGENTRIES), ENT_QUOTES, 'UTF-8'): '',
		'PC_LIST_NEXT'			=> ($more_rows === TRUE) ? 'Next' : '',
		'PC_LIST_PREV_LINK'		=> ($listfrom > 0) ? htmlspecialchars($_SERVER['PHP_SELF'] . '?listfrom=' . max($listfrom-OVRIMGENTRIES, 0), ENT_QUOTES, 'UTF-8'): '',
	...
	while (($myrow = mysql_fetch_array($result)) && $count < OVRIMGENTRIES) {
		$page->set_var(array(
			...
			'PC_CAR_IMG_SRC'	=> $path_to_root . 'images/cars/' . $myrow['carid'] . '_tn.jpg'
		));
