...
*/
	...
	if ($_SESSION['logged'] == TRUE) {
		...
		//$page_content = 'forum_message_edit.ihtml';
	} else {
	...
	// Set up page header.
	$page->set_var(array(
		'PB_PAGETITLE'		=> 'Open Racing Simulator — Racing Board, edit forum message',
		'PB_DESCRIPTION'	=> 'Edit message in Open Racing Simulator — Racing Board forum',
	...
	if ($_SESSION['logged'] == TRUE) {
		...
		if (isset($_GET['msgid'])) {
			$msg_id_for_db = quoteString(intval(removeMagicQuotes($_GET['msgid'])));
			$sql = "SELECT * FROM " . $forum_tablename . " WHERE id=" . $msg_id_for_db;
			...
			if ($myrow = mysql_fetch_array($result)) {
				...
				{
					$page->set_var(array(
						'PC_SUBJECT'		=> htmlentities($myrow['subject']),
						'PC_ID_MESSAGE'		=> $myrow['id'],
						'PC_ID_THREAD'		=> $myrow['id_thread'],
						'PC_MESSAGE'		=> htmlentities($myrow['text']),
						'PC_FORUM_EDIT'		=> './showthread.php'
					));
					$page_content = 'forum_message_edit.ihtml';
					$page->set_file('PAGE_CONTENT_T', $page_content);
					
					// Poll list.
					$page->set_block('PAGE_CONTENT_T', 'message_poll', 'msg_poll');
					$page->set_block('message_poll', 'message_poll_options', 'msg_poll_opt');
					if ($_SESSION['usergroup'] == 'admin' && $myrow['id_parent'] == 0) {
						// Which poll is assigned to this topic?
						$sql = "SELECT * FROM " . $forumpoll_tablename . " WHERE threadid=" . $msg_id_for_db;
						$result = mysql_query($sql);
						$preselectedpoll = intval(0);
						if ($myrow = mysql_fetch_array($result)) {
							$preselectedpoll = $myrow['pollid'];
						}
						...
						if (mysql_num_rows($result) > 0) {
							while ($myrow = mysql_fetch_array($result)) {
								$page->set_var(array(
									...
									'PC_POLL_SELECTED'	=> ($myrow['pollid'] == $preselectedpoll) ? 'selected' : ''
								));
