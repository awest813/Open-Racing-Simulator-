...
*/
		...
		if ($_SESSION['usergroup'] == 'admin') {
			if (isset($_POST['poll_submit'])) {
				if ($formerrors = checkPollInput()) {
					...
					commitPollInput($poll_tablename, $poll_answer_tablename, $_SESSION['uid']);
					$page_content = 'admin_poll_create_ok.ihtml';
				}
			} else {
				$page_content = 'admin_poll_create.ihtml';
			}
		} else {
	...
	if ($_SESSION['logged'] == TRUE) {
		...
		if (isset($_POST['poll_submit']) && $formerrors > 0) {
			...
			$page->set_var(array(
				'PC_VALUE_STARTDATE'	=> date("Y-m-d H:i:s"),
				'PC_VALUE_ENDDATE'		=> date("Y-m-d H:i:s", time() + POLL_DURATION)
			));
		}
