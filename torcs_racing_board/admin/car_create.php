...
*/
		...
		if ($_SESSION['usergroup'] == 'admin') {
			if (isset($_POST['car_submit'])) {
				if ($formerrors = checkCarInput()) {
					// Errors.
					$page_content = 'admin_car_create.ihtml';
				} else {
					commitCarInput($car_tablename, $path_to_root);
	...
	// Set up page header.
	$page->set_var(array(
		'PB_PAGETITLE'		=> 'Open Racing Simulator — Racing Board Car Creation Page',
		'PB_DESCRIPTION'	=> 'Create a car for the Open Racing Simulator racing board',
		'PB_AUTHOR'			=> 'Bernhard Wymann',
		'PB_KEYWORDS'		=> 'Open Racing Simulator, racing, berniw, Bernhard, Wymann, Championship, World, car, create',
	...
	if ($_SESSION['logged'] == TRUE) {
		...
		$page->set_var(array(
			...
			'PC_CREATECARPAGE'		=> htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8')
		));

		if (isset($_POST['car_submit']) && $formerrors > 0) {
			$page->set_var(array(
