...
*/
		...
		if ($_SESSION['usergroup'] == 'admin') {
			if (isset($_POST['news_submit'])) {
				if ($formerrors = checkNewsInput()) {
					// Errors.
					$page_content = 'admin_news_create.ihtml';
				} else {
					commitNewsInput($news_tablename, $path_to_root);
	...
	// Set up page header.
	$page->set_var(array(
		'PB_PAGETITLE'		=> 'Open Racing Simulator — Racing Board Write News Page',
		'PB_DESCRIPTION'	=> 'Write News for the Open Racing Simulator racing board',
		'PB_AUTHOR'			=> 'Bernhard Wymann',
		'PB_KEYWORDS'		=> 'Open Racing Simulator, racing, berniw, Bernhard, Wymann, Championship, World, create, news',
	...
	if ($_SESSION['logged'] == TRUE) {
		...
		$page->set_var(array(
			...
			'PC_CREATENEWSPAGE'		=> htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8')
		));

		if (isset($_POST['news_submit']) && $formerrors > 0) {
			$page->set_var(array(
