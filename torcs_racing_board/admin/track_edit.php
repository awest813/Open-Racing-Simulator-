...
*/
	...
	if ($_SESSION['logged'] == TRUE) {
		...
		if ($_SESSION['usergroup'] == 'admin' &&
			(isset($_POST['track_submit']) || isset($_GET['edittrackid']))
		) {
			if (isset($_POST['track_submit'])) {
				if ($formerrors = checkTrackInput()) {
					// Errors.
					$page_content = 'admin_track_edit.ihtml';
				} else {
					updateTrackInput($track_tablename, $path_to_root);
	...
	// Set up page header.
	$page->set_var(array(
		'PB_PAGETITLE'		=> 'Open Racing Simulator — Racing Board Track Edit Page',
		'PB_DESCRIPTION'	=> 'Edit track data for the Open Racing Simulator racing board',
		'PB_AUTHOR'			=> 'Bernhard Wymann',
		'PB_KEYWORDS'		=> 'Open Racing Simulator, racing, berniw, Bernhard, Wymann, Championship, World, track, edit',
	...
	if ($_SESSION['logged'] == TRUE) {
		...
		$page->set_var(array(
			...
			'PC_CREATETRACKPAGE'	=> htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8')
		));

		if (isset($_POST['track_submit']) && $formerrors > 0 && isset($_POST['track_id'])) {
			$page->set_var(array(
		...
		} elseif (isset($_GET['edittrackid'])) {
			$id_for_db = quoteString(intval(removeMagicQuotes($_GET['edittrackid'])));
			$sql = "SELECT * FROM $track_tablename WHERE trackid=" . $id_for_db;
			...
			if ($myrow = mysql_fetch_array($result)) {
				$page->set_var(array(
					...
					'PC_TRACK_TYPE_DIRT'	=> (strcmp($myrow['type'], 'dirt') == 0) ? 'selected' : '',
					'PC_TRACK_TYPE_OVAL'	=> (strcmp($myrow['type'], 'oval') == 0) ? 'selected' : '',
					'PC_TRACK_TYPE_ROAD'	=> (strcmp($myrow['type'], 'road') == 0) ? 'selected' : '',
