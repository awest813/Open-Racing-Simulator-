...
*/
	...
	require_once($path_to_root . 'lib/functions_sshot.php');
	...
	if ($_SESSION['logged'] == TRUE) {
		...
		if ($_SESSION['usergroup'] == 'admin') {
			if (isset($_POST['sshot_submit'])) {
				if ($formerrors = checkSShotInput()) {
					// Errors.
					$page_content = 'admin_sshot_create.ihtml';
				} else {
					commitSShotInput($sshot_tablename, $path_to_root);
					$page_content = 'admin_sshot_create_ok.ihtml';
				}
			} else {
				$page_content = 'admin_sshot_create.ihtml';
			}
	...
	// Set up page header.
	$page->set_var(array(
		'PB_PAGETITLE'		=> 'Open Racing Simulator — Racing Board Screenshot Upload Page',
		'PB_DESCRIPTION'	=> 'Upload a screenshot on the Open Racing Simulator racing board',
		'PB_AUTHOR'			=> 'Bernhard Wymann',
		'PB_KEYWORDS'		=> 'Open Racing Simulator, racing, berniw, Bernhard, Wymann, Championship, World, create, screenshot',
	...
	if ($_SESSION['logged'] == TRUE) {
		...
		$page->set_var(array(
			...
			'PC_CREATESSHOTPAGE'		=> htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8')
		));

		if (isset($_POST['sshot_submit']) && $formerrors > 0) {
			$page->set_var(array(
				'PC_SSHOT_DESCRIPTION'	=> isset($_POST['sshot_description']) ? htmlentities(removeMagicQuotes($_POST['sshot_description'])) : "",
				'PC_SSHOT_IMG_SRC'		=> isset($_FILES['sshot_image']) ? htmlentities(removeMagicQuotes($_FILES['sshot_image']['name'])) : ""
			));
		} else {
			$page->set_var(array(
				'PC_SSHOT_DESCRIPTION'	=> "",
				'PC_SSHOT_IMG_SRC'		=> ""
			));
		}
	} else {
