...
*/
	...
	if ($_SESSION['logged'] == TRUE) {
		...
		if ($_SESSION['usergroup'] == 'admin') {
			$page_content = 'admin_loginlog_list.ihtml';
		}
	} else {
	...
	// Set up page header.
	$page->set_var(array(
		'PB_PAGETITLE'		=> 'Open Racing Simulator — Racing Board User List',
		'PB_DESCRIPTION'	=> 'User list of the Open Racing Simulator racing board',
		'PB_AUTHOR'			=> 'Bernhard Wymann',
		'PB_KEYWORDS'		=> 'Open Racing Simulator, racing, berniw, Bernhard, Wymann, Championship, World, Board, User, Users, List',
	...
	if ($_SESSION['usergroup'] == 'admin') {
		...
		$sql = "SELECT * FROM $loginlog_tablename " .
			   "ORDER BY time DESC LIMIT $listfrom_for_db,$listentries_for_db";

		$result = mysql_query($sql);
		$more_rows = (mysql_num_rows($result) > LISTENTRIES);
		$page->set_var(array(
			'PC_LIST_NEXT_LINK'		=> ($more_rows === TRUE) ? htmlspecialchars($_SERVER['PHP_SELF'] . '?listfrom=' . ($listfrom+LISTENTRIES), ENT_QUOTES, 'UTF-8'): '',
			'PC_LIST_NEXT'			=> ($more_rows === TRUE) ? 'Forward' : '',
			'PC_LIST_PREV_LINK'		=> ($listfrom > 0) ? htmlspecialchars($_SERVER['PHP_SELF'] . '?listfrom=' . max($listfrom-LISTENTRIES, 0), ENT_QUOTES, 'UTF-8'): '',
		...
		while (($myrow = mysql_fetch_array($result)) && $results < LISTENTRIES) {
			$page->set_var(array(
				...
				'PC_USER_TIME'		=> mysqlToTime($myrow['time']),
				'PC_USER_COUNT'		=> $myrow['count'],
				'PC_USER_STATE'		=> $myrow['pf'],
				'PC_COLOR'			=> ($myrow['pf'] != 'failed') ? (($toggle == 0) ? COLOR_TB1 : COLOR_TB2) : COLOR_TB_ERR
			));
