...
*/
	...
	if ($_SESSION['logged'] == TRUE) {
		...
		if ($_SESSION['usergroup'] == 'admin' && existsEntry($event_tablename, 'eventid', $eventid)) {
			if (isset($_POST['races_submit'])) {
				if ($formerrors = checkRacesInput()) {
					// Errors.
					$page_content = 'admin_races_create.ihtml';
				} else {
					commitRacesInput($race_tablename, $event_tablename, $track_tablename, $path_to_root);
					$page_content = 'admin_races_create_ok.ihtml';
				}
			} else {
				$page_content = 'admin_races_create.ihtml';
			}
	...
	// Set up page header.
	$page->set_var(array(
		'PB_PAGETITLE'		=> 'Open Racing Simulator — Racing Board Races Creation Page',
		'PB_DESCRIPTION'	=> 'Create races for the Open Racing Simulator racing board',
		'PB_AUTHOR'			=> 'Bernhard Wymann',
		'PB_KEYWORDS'		=> 'Open Racing Simulator, racing, berniw, Bernhard, Wymann, Championship, World, event, race, create',
	...
	if ($_SESSION['logged'] == TRUE) {
		...
		$page->set_var(array(
			...
			'PC_CREATERACESPAGE'	=> htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8')
		));

		if ($_SESSION['usergroup'] == 'admin' && $page_content == 'admin_races_create.ihtml') {
			...
			if (isset($_POST['races_submit'])) {
				$page->set_var(array(
					'PC_EVENT_ID'					=> $eventid,
					'PC_BASE_RACES_START'			=> isset($_POST['base_races_start']) ? htmlentities(removeMagicQuotes($_POST['base_races_start'])) : "",
					'PC_RACE_SUBMISSION_DURATION'	=> isset($_POST['race_submission_duration']) ? htmlentities(removeMagicQuotes($_POST['race_submission_duration'])) : "",
					'PC_RACE_RACING_DURATION'		=> isset($_POST['race_racing_duration']) ? htmlentities(removeMagicQuotes($_POST['race_racing_duration'])) : "",
					'PC_RACE_MAINTENANCE_DURATION'	=> isset($_POST['race_maintenance_duration']) ? htmlentities(removeMagicQuotes($_POST['race_maintenance_duration'])) : "",
				));

				for ($i = 1; $i <= RACES_PER_EVENT; $i++) {
					$page->set_var(array(
						'PC_EVENT_RACES_SEL'	=> (isset($_POST['event_races']) && $i == intval(removeMagicQuotes($_POST['event_races']))) ? 'selected' : "",
						'PC_EVENT_RACES_NB'		=> $i
					));
				...
				$page->set_var(array(
					'PC_EVENT_ID'					=> $eventid,
					'PC_BASE_RACES_START'			=> "",
					'PC_RACE_SUBMISSION_DURATION'	=> "",
					'PC_RACE_RACING_DURATION'		=> "",
					'PC_RACE_MAINTENANCE_DURATION'	=> "",
				));
