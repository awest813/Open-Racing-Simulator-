...
*/
	...
	$error = updateRobot($event_tablename, $event_team_table, $team_tablename, $race_tablename, $path_to_root);
	...
	// Set up page header.
	$page->set_var(array(
		'PB_PAGETITLE'		=> 'Open Racing Simulator — Racing Board Robot Submission Status',
		'PB_DESCRIPTION'	=> 'Status of the robot submission on the Open Racing Simulator Racing Board',
		'PB_AUTHOR'			=> 'Bernhard Wymann',
		'PB_KEYWORDS'		=> 'Open Racing Simulator, racing, berniw, Bernhard, Wymann, Championship, World, event, race, competition, view, robot, submission, submit',
	...
	if ($_SESSION['usergroup'] == 'racer' &&
		...
		if ($error) {
			$page->set_file('PAGE_CONTENT_T', 'event_robot_submitted_notok.ihtml');
			$page->set_var(array(
				'PC_SUBMITTED_FILE_SIZE'	=> intval($_FILES['update_robot_package']['size']),
				'PC_ALLOWED_FILE_SIZE'		=> ROBOT_MODULE_SIZE,
				'PC_SUBMITTED_MIME_TYPE'	=> htmlentities($_FILES['update_robot_package']['type']),
			));
		} else {
			$page->set_file('PAGE_CONTENT_T', 'event_robot_submitted_ok.ihtml');
		}
	}
