...
*/
	...
	if (isset($_GET['eventid']) && isset($_GET['raceid']) && isset($_GET['teamid']) && $_SESSION['usergroup'] == 'racer') {
		...
		$sql = "SELECT t.name AS tname, e.name AS ename, t.modulename AS modulename, " .
			   "r.robot_submission_start AS rs, r.robot_submission_end AS re " .
		...
		if (mysql_num_rows($result) == 1 && $myrow = mysql_fetch_array($result)) {
			...
			if ($time1 <= 0 && $time2 >= 0) {
				$page->set_file('PAGE_CONTENT_T', 'event_upload_robot.ihtml');
				$page->set_var(array(
					'PC_EVENT_ID'				=> $eventid,
