...
*/
	...
	// Set up page header.
	$page->set_var(array(
		'PB_PAGETITLE'		=> 'Open Racing Simulator — Racing Board Register Team Status',
		'PB_DESCRIPTION'	=> 'Status of the team registration on the Open Racing Simulator Racing Board',
		'PB_AUTHOR'			=> 'Bernhard Wymann',
		'PB_KEYWORDS'		=> 'Open Racing Simulator, racing, berniw, Bernhard, Wymann, Championship, World, event, race, competition, view, robot, submission, submit, register, team',
	...
	if ($_SESSION['usergroup'] == 'racer' &&
		...
		if ($error) {
			$page->set_file('PAGE_CONTENT_T', 'event_team_registered_notok.ihtml');
			$page->set_var(array(
				'PC_SUBMITTED_FILE_SIZE'	=> intval($_FILES['robot_package']['size']),
				'PC_ALLOWED_FILE_SIZE'		=> ROBOT_MODULE_SIZE,
				'PC_SUBMITTED_MIME_TYPE'	=> htmlentities($_FILES['robot_package']['type']),
			));
		} else {
			$page->set_file('PAGE_CONTENT_T', 'event_team_registered_ok.ihtml');
		}
	}
