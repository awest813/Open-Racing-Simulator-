...
*/
	...
	if ($_SESSION['logged'] == TRUE) {
		...
		if ($_SESSION['usergroup'] == 'spectator') {
			$errors = checkAnswers(1, $answers_tablename, $quizz_questions_tablename, $quizz_tablename);
			if ($errors == 0) {
				$page_content = 'account_spectator_to_racer_pass.ihtml';
				// Write to db.
				toRacer($user_tablename, $_SESSION['uid']);
			} else {
				$page_content = 'account_spectator_to_racer_fail.ihtml';
			}
		}
	} else {
	...
	// Set up page header.
	$page->set_var(array(
		'PB_PAGETITLE'		=> 'Open Racing Simulator — Racing Board racers entry quizz result page',
		'PB_DESCRIPTION'	=> 'Your result of the racer entry quizz for the Open Racing Simulator racing board',
		'PB_AUTHOR'			=> 'Bernhard Wymann',
		'PB_KEYWORDS'		=> 'Open Racing Simulator, racing, berniw, Bernhard, Wymann, Championship, World, result, entry, quizz, exam, test',
